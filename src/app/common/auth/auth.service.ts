...
export class AuthService {
  private readonly _betterAuthClient: BetterAuthClientService =
    inject(BetterAuthClientService);
  private readonly _router: Router = inject(Router);
  private readonly _authClient =
    this._betterAuthClient.getClient();
  ...
  public twoFactorEnableData: WritableSignal<
    ...
    | undefined
  > = signal(undefined);
  public jwtToken: WritableSignal<string | undefined> =
  ...
  public signInWithPasskey(autoFill: boolean) {
    return from(
      this._authClient.signIn.passkey({
  ...
  // ==========================================================================
  public getJwtToken() {
    return from(this._authClient.token()).pipe(
      map((res) => {
        if (res.error) throw res.error;
        return res.data;
      }),
  ...
  // ==========================================================================
  public getSession() {
    return from(this._authClient.getSession()).pipe(
      map((res) => {
        if (res.error) throw res.error;
