...
export class TwoFactorComponent {
  ...
  public onVerify(): void {
    ...
    this._authService.verifyTwoFactorTOTP(this.form.value.code).subscribe({
      next: (): void => {
  ...
  public onVerifyWithBackupCode(): void {
    if (this.form.invalid) return;
    this._authService.verifyTwoFactorBackupCode(this.form.value.code).subscribe({
      next: (): void => {
        this.errorMessage.set('');
        this._router.navigate(['/redirect-to-home']);
      },
      error: (err: any): void => {
        this.errorMessage.set(err.message || 'Invalid backup code');
      }
    });
  }
}
